...
하나의 표현 언어를 생성할 필요성으로 생겨난 것이다. 버전들은 텍스트 위주의 정보전달 목적에서  
많은 정보를 표현하기를 원하게 되며 일반 사용자들에게도 브라우저가 보편화되기 시작하며 발전하였다.  
HTML4.01은 HTML의 안정기이며 CSS(Cascading Style Sheet)로 디자인적 요소를 구분하고 HTML에서는 웹페이지의  
전반적인 구조만을 명시하는 것을 원칙으로 하게 된다.  
그리고 이때 W3C에서는 HTML은 기술적 표현의 한계를 맞아 더이상 HTML을 발전시키지 않게 된다.   
...
\<!doctype html\>  html5로 작성된 웹 문서, 처리할 문서가 어떤 유형인지 알려주는 역할    
