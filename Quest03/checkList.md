...
|    | var | let |
|----|-----|-----|
|스코프|전역,지역 다 사용됨(함수 외부 선언 전역, 함수 내 지역)|블록 스코프, 선언된 변수는 해당 블록 내에서만
|변화|재선언 및 업데이트 가능|업데이트는 가능하나 재선언은 불가|
|호이스팅| undefined로 변수 초기화 | 변수 초기화하지 않음(사용시 에러 발생)|

- var는 이미 정의되어 있다는 것을 인식하지 못하는 문제가 있다, 의도하지 않은 값이 출력될 수 있으며,  
코드에 버그가 발생할 수 있다. 그 이유로 let이 선호된다. let은 해당 스코프 내에서만 존재하므로,  
이전에 동일한 변수명을 사용한 적이 있는지 없는지 신경쓸 필요 없다.
※ 호이스팅이란, 인터프리터가 변수와 함수의 메모리 공간을 선언 전에 미리 할당 하는것  

## 5. 자바스크립트의 익명 함수는 무엇인가요?  
변수에 함수의 코드를 저장하는 대신 변수명을 사용하지 않는다. 대신 변수명을 마치 함수명처럼 사용해서 함수를 호출하거나 변수값을 이동시키는데 사용할 수 있다.  
<script>  
 var 변수명 = function(매개변수){ 실행문; };  
</script>  
익명함수는 함수코드가 변수명에 저장된 형태, 따라서 변수값으로 구성된 함수 코드를 다른 변수명에 마치 변수를 대입하듯 이동가능  
※ 익명함수는 호이스팅은 작동하지 않고, 일반 함수만 작동함  

 ### 5-1. 자바스크립트의 Arrow function은 무엇일까요?  
기존함수의 축약버전  
var sum = function(a,b){ return a+b; };  
==> var sum = (a,b) => a+b; 로 변환  
1. 중괄호 없이 작성 : (..args) => experession  
화살표 오른쪽에 표현식, 함수는 표현식을 평가하고 결과를 반환한다.  
2. 중괄호와 함께 작성 : (..args) => { body }  
본문이 여러줄로 구성되었다면 중괄호 사용, 다만, 이 경우는 반드시 return 지시자 사용하여 반환 값을 명시해야 한다.  
※ 인수가 하나도 없을 땐 괄호를 비워놓아야하고, 괄호를 생략할 수 없다.  
