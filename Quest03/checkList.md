...
var  
함수 외부에서 선언되면 전역(global), 함수 블록 외부에 선언된 모든 변수를 전체 window에서 사용, 함수 내에 선언되면 함수 스코프  
이미 정의되어있는 변수를 인식하지 못하여 의도하지 않은 값이 출려될 수 있으며, 코드에 버그가 발생될 수도 있음
let
var의 문제점을 보완해줌  
블록 스코프 내에서 작동한다. 선언된 변수는 해당 블록 내에서만 사용 가능  
업데이트할 수 있지만 재선언 불가, let을 사용할 때는 변수가 해당 스코프 내에서만 존재하므로, 이전에 동일한 변수명을 사용한 적이 있는지 신경 쓸 필요 없다. 또한 변수는 한 스코프 내에서 두 번 이상 선언될 수 없기 때문에 앞에서 얘기한 var의 문제는 발생하지 않는다.
